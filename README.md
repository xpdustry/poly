# poly

[![Downloads](https://img.shields.io/github/downloads/xpdustry/poly/total?color=008080&label=Downloads)](https://github.com/xpdustry/poly/releases)
[![Mindustry 8.0](https://img.shields.io/badge/Mindustry-8.0-008080)](https://github.com/Anuken/Mindustry/releases)
[![Discord](https://img.shields.io/discord/519293558599974912?color=008080&label=Discord)](https://discord.xpdustry.com)

## Description

Redirect players.
POC for a global event server redirector.

## Installation

The plugin requires:

- Java 25 or above.
- Mindustry v157 or above.

## Building

- `./gradlew :shadowJar` to compile the plugin into a usable jar (will be located at `build/libs/poly.jar`).
- `./gradlew :runMindustryServer` to run the plugin in a local Mindustry server.
- `./gradlew :runMindustryDesktop` to start a local Mindustry client that will let you test the plugin.
